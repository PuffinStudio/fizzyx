...
- If the exact derived branch already exists, enter it instead of trying to recreate it.
...
- Unchanged files explicitly captured in the branch baseline are warnings, not blockers.
...
- A receipt under `.git/fizzyx/ready/` bound to the current HEAD and check configuration.

A failed readiness check exits non-zero. If local state was lost, `dev ready` remains
conservative until the user inspects the worktree and runs `dev baseline accept`.
...
- Default mode is a read-only preview: keep the current branch and report branches that
  could be deleted.
...
- Remove the deleted branch's baseline and readiness receipt from `.git/fizzyx/`.
...
The Git workflow does not require a specific Fizzy column vocabulary. Custom boards use
`fizzyx flow columns` to discover real IDs and
`fizzyx flow move <card> <column-id-or-name>` to transition cards. BACKLOG, READY, IN PROGRESS, and REVIEW
belong to the bundled Fizzyx preset. `flow start` uses the configured in-progress column
ID; `flow review` is a preset convenience command. Normal flow commands never provision or
rename columns. Explicit initialization may install the preset when no flow mapping exists.

| Flow action                     | Dev integration                                                            |
| ------------------------------- | -------------------------------------------------------------------------- |
| `fizzyx flow move <card> <col>` | Moves a card on any custom or preset board                                 |
| `fizzyx flow start <card>`      | Uses configured in-progress role; may suggest the matching `dev start`     |
...
When `dev start --card 123` runs, fizzyx stores lightweight metadata in local Git config:

```sh
git config --local branch.feature/card-123-payment-coupon.fizzyx-card 123
git config --local branch.feature/card-123-payment-coupon.fizzyx-kind feature
git config --local branch.feature/card-123-payment-coupon.fizzyx-base main
```

This metadata is for guidance only. It never modifies `.fizzyx.yaml`. Legacy `dev.branches`
configuration remains readable for compatibility but is no longer written. If local Git
config is lost, `dev status` can still infer the card number from a standard
`*/card-<number>-*` branch name; readiness must always be regenerated by rerunning checks.

### Storage Boundaries

| State                              | Location                    | Shared in Git |
| ---------------------------------- | --------------------------- | ------------- |
| Team branch/check/promotion policy | `.fizzyx.yaml`              | Yes           |
| Branch/card association            | repository-local Git config | No            |
| Baselines and readiness receipts   | `.git/fizzyx/`              | No            |
| Card drafts in a Git repository    | `.git/fizzyx/drafts/`       | No            |
| Card drafts outside Git            | OS user state directory     | No            |

On Windows, configured checks and promotion commands run through `%ComSpec%`; external
draft state uses `%LOCALAPPDATA%`. On Unix-like systems, checks retain `bash -c`, and
external drafts use `$XDG_STATE_HOME` or `~/.local/state`.
...
fizzyx skill update dev-workflow [--global]
fizzyx skill doctor
```

Top-level `fizzyx init` initializes project configuration only. It does not copy skills.
Project materialization is explicit under `.agents/skills`; global materialization is
explicit under `~/.agents/skills`. `skill add` pins and materializes one project skill.
...
1. Run `fizzyx dev status --agent`. Inspect pre-existing changes and explicitly capture them
   with `fizzyx dev baseline accept` before making task edits.
...
`fizzyx init` creates `AGENTS.md` when it is missing and otherwise appends or refreshes a
marker-delimited FizzyX section without replacing project-owned instructions. The generated
section provides the compact `flow` + `dev` loop; the fuller Git-specific rules remain:

```md
## Git workflow for agents

- Use `fizzyx flow` for remote card state and `fizzyx dev` for local Git state.
- Inspect custom board columns with `fizzyx flow columns`; use `fizzyx flow move` when a board
  does not use the bundled READY/REVIEW preset.
