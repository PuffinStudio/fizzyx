...
import type { PlannerCard, PlannerSnapshot, SelectCard } from "./planner-types";
...
export function MyCardsView({
	...
	const myCards = snapshot.identity
		? snapshot.cards.filter(
				(card) =>
					card.assignees.some((user) => user.id === snapshot.identity?.id) ||
			...
			<section className="grid min-w-0 grid-cols-6 gap-3 overflow-hidden">
				{lanes.map((lane, i) => (
					...
					>
						<LaneColumn
							title={lane.label}
							cards={myCards.filter((card) => card.lane === lane.key)}
							onSelect={onSelect}
						/>
					</motion.div>
				))}
