	...
	content: `// Generated by fizzyx. Do not edit.\n\nexport const adminPlan = ${JSON.stringify(
		{
			...
			resources: plan.resources.map(
				({ key, id, label, path, group, order, icon, hidden, presentation }) => ({
					key,
					id,
					label,
					path,
...
const navigationAwareShellTemplate = (template: string): string => {
	...
	return template
		...
		.replace(
			sidebarContent,
			'        <SidebarContent className="p-2"><AdminNavigation groups={adminPlan.navigation.groups.map((group) => ({ ...group, items: group.items.map((item) => ({ key: item.resourceKey, label: item.label, href: item.path, icon: item.icon as never })) }))} /></SidebarContent>',
		);
};
