...
import { generateTanstackQuery } from "./codegen-tanstack-query";
...
const DEFAULT_FETCH_OPTIONS: GenFileOptions = {
	...
	stateManagementName: "queries.ts",
};
		...
		Effect.gen(function* () {
			try {
				...
				if (options?.stateManagement === "tanstack-query") {
					const smFile =
						options.stateManagementName ??
						DEFAULT_FETCH_OPTIONS.stateManagementName ??
						"queries.ts";
					const queriesCode = generateTanstackQuery(spec, apiImportPath, typesImportPath, hasTypes);
					files.push({ path: smFile, content: queriesCode });
				}

				const stateMgmtExport =
					options?.stateManagement === "tanstack-query"
						? `export * from "./${(options.stateManagementName ?? DEFAULT_FETCH_OPTIONS.stateManagementName ?? "queries.ts").replace(/\.ts$/, "")}"`
						: undefined;
				...
				files.push({
					...
					content: [
						...
						...(stateMgmtExport ? [stateMgmtExport] : []),
						"",
					].join("\n"),
				});
