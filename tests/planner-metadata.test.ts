...
import {
	...
	planMetadataRepair,
	toPlannerCard,
} from "../src/use-cases/planner-transform";
...
Ship it`);
	...
	expect("api_status" in parsed.metadata).toBe(false);
...
test("parsePlannerTags leaves api_status and skill tags as non-standard tags", () => {
	...
	expect(parsed.other).toEqual(["api_status:not_connected", "skill:tdd", "skill:playwright"]);
});
...
<p>HTML body.</p>`,
	...
	expect("api_status" in card.metadata).toBe(false);
...
test("analyzePlannerHealth reports missing metadata and workflow risks", () => {
	const cards: PlannerCard[] = [
		...
		makeCard({
			number: 4,
			lane: "ready",
			metadata: { depends_on: [2], blocks: [], owner: "ellen" },
